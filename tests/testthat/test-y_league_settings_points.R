...
with_mock_api({
    testthat::test_that("request returns valid response and is parsed to a tibble", {
        uri <-
            "https://fantasysports.yahooapis.com/fantasy/v2/league/321.l.35439/settings?format=json"
        ...
        testthat::expect_equal(purrr::pluck(r_parsed, 1, "scoring_type"), "point")

        df <- .point_league_settings(r_parsed)
        ...
        # check row names
        y <- c("league_meta", "league_settings", "roster_positions", "stat_modifiers")
        testthat::expect_identical(df$info, y)
