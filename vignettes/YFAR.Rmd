...
### Create Token with y_create_token
...
After creating your token you should not need to run `y_create_token()` again.  You can just keep refreshing your existing token.
...
Get your game data
  ...
  > A note on terminology for those who are wondering. 
  >
  > A game in this context is the fantasy sport you are playing or getting data for.  I.E. 2021 fantasy hockey is a game.
  >
...
Having said all that lets get some game data.
```{r, echo=FALSE}
# Create a dummy token class to pacify functions checking for a token.
my_token <-
    structure(
        list(),
...
```{r}
team_trans %>%
  select(type, timestamp, player_full)
```
...
Top 10 players by actual rank ("OR")
```{r y_players top 10, include=FALSE}
with_mock_api({
top_10_ar_players <- y_players("411.l.1239", token_name = my_token, number_of_players = 10, start = 0, sort = "OR", status = "ALL", debug = FALSE)
})
```
...
### y_matchups vs y_scoreboard

Well what's the difference between y_matchups and y_scoreboard?  They are similar and "under the hood" they use the same parsing functions but y_matchups takes a team_id where y_scoreboard takes a league_id.
...
```{r y_matchup week 8, include=FALSE}
with_mock_api({
...
```{r, include=FALSE}
with_mock_api({
league_matchups <- y_scoreboard("411.l.1239", my_token)
})
```
...
Multiple weeks
    ...
    y_scoreboard("411.l.1239", my_token, week = c(4, 5))
...
```{r team roster with date, include=FALSE}
with_mock_api({
team_roster_w_date <- y_rosters(id = "411.l.1239.t.1", token_name = my_token, game_date = "2021-10-12")
})
```

```{r, eval=FALSE}
team_roster_w_date <- y_rosters(id = "411.l.1239.t.1", token_name = my_token)
```
...
```{r, include=FALSE}
with_mock_api({
team_1_roster <- y_rosters(id = "411.l.1239.t.1", token_name = my_token)
})
```
...
```{r}
team_1_roster %>%
  dplyr::select(player_key, player_full)
```
...
```{r, eval=FALSE}
# Not run
  my_player_stats <- y_player_stats(players = team_1_roster$player_key, token_name = my_token)
```
